...
public class GradientView extends View {
    ...
    private void init(AttributeSet attrs) {
        ...
        if (attrs != null) {
            TypedArray typedArray = getContext().obtainStyledAttributes(attrs, com.example.klaus.mylight.R.styleable.GradientView);
            for (int i = 0, n = typedArray.getIndexCount(); i < n; i++) {
                int index = typedArray.getIndex(i);
                switch (index) {
                    case com.example.klaus.mylight.R.styleable.GradientView_radius:
                        setRadius(typedArray.getDimensionPixelSize(index, 0));
                        break;
                    case com.example.klaus.mylight.R.styleable.GradientView_pointerDrawable:
                        setPointerDrawable(typedArray.getDrawable(index));
                        break;
                    case com.example.klaus.mylight.R.styleable.GradientView_lockPointerInBounds:
